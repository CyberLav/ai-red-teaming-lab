...
### Step 6: Watch out for a preexisting WSL distro shadowing the new one

Problem hit: there was already an old Kali Linux WSL install from a previous project. Typing `wsl` in PowerShell launched the default distro, which was still Kali, not the freshly installed Ubuntu. Running `apt update` inside it failed with:
...
Diagnosis, list all installed distros and see which is default:

```
wsl -l -v
```

Fix, launch Ubuntu explicitly:

```
wsl -d Ubuntu
```

Confirm you are actually in it:

```
cat /etc/os-release
```

It should say Ubuntu, not Kali.

The old Kali install was removed entirely, since it was not needed and was just causing confusion:

```
wsl --unregister kali-linux
```

This permanently deletes everything inside that distro. Check `wsl -l -v` for the exact name first, and only do this if there is nothing in it you need.

### Step 7: Set up Python and Garak inside Ubuntu
...
### Step 8: Confirm WSL can actually reach Ollama
